        ...
        <div className="max-w-7xl mx-auto px-4 py-12 space-y-8">
            <header>
                <h1 className="text-4xl font-light text-zinc-900">
                    {isFarmer ? "Sales " : "My "}
                    <span className="font-bold text-emerald-600">History</span>
                </h1>
                <p className="text-zinc-500 mt-2 italic">
                    {isFarmer ? "Track your successful sales and fulfillment status." : "Monitor your recent purchases and delivery updates."}
                </p>
            </header>

            {loading ? (
                <div className="py-20 text-center text-zinc-400 animate-pulse">Retrieving order records...</div>
            ) : orders.length === 0 ? (
                <div className="py-20 text-center bg-zinc-50 rounded-[3rem] border border-dashed border-zinc-200">
                    <Package className="w-12 h-12 text-zinc-300 mx-auto mb-4" />
                    <p className="text-zinc-500 font-medium">No records found yet. Confirmed deals will appear here.</p>
                </div>
            ) : (
                        ...
                        <div key={order.order_id} className="bg-white p-8 rounded-[2.5rem] border border-zinc-100 shadow-xl shadow-zinc-200/50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 group hover:border-emerald-100 transition-all">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-4 text-zinc-400 text-sm">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span>{isFarmer ? `Buyer: ${order.buyer_name}` : `Farmer: ${order.farmer_name}`}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-8 lg:gap-12 w-full lg:w-auto border-t lg:border-t-0 lg:border-l border-zinc-100 pt-6 lg:pt-0 lg:pl-12">
                                <div className="space-y-1">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Transaction Value</p>
                                    <p className="text-2xl font-black text-zinc-900">₹{order.final_price}<span className="text-xs ml-0.5 text-zinc-400">Total</span></p>
                                </div>
                                <div className="space-y-1">
